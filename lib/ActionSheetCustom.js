...
class ActionSheet extends React.Component {
  ...
  _createButton (title, index) {
    ...
    const buttonTextStyle = cancelButtonIndex === index ? styles.cancelButtonText : styles.buttonText
    return (
      <TouchableOpacity
        key={index}
      ...
      >
        {React.isValidElement(title) ? title : (
          <Text style={[buttonTextStyle, {color: fontColor}]}>{title}</Text>
        )}
      </TouchableOpacity>
    )
  }
      ...
      >
        <View style={[styles.wrapper]}>
          ...
          >
            {this._renderTitle()}
            {this._renderMessage()}
            <ScrollView contentContainerStyle={styles.optionContainer} scrollEnabled={this.scrollEnabled}>{this._renderOptions()}</ScrollView>
